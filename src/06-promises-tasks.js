...
function chainPromises(array, action) {
  const promisify = (promises, _action, result) => new Promise((resolve) => {
    if (promises.length) {
      promises
        .shift()
        .then((res) => {
          resolve(
            promisify(promises, _action, result ? _action(result, res) : res),
          );
        })
        .catch(() => resolve(promisify(promises, _action, result)));
    } else {
      resolve(result);
    }
  });

  return promisify(array, action);
}
