...
function getFactorial(n) {
  return new Array(n).fill(0).map((_, idx) => idx + 1).reduce((acc, el) => acc * el, 1);
}
...
function getSumBetweenNumbers(n1, n2) {
  return new Array(n2 - n1 + 1).fill(0).map((_, idx) => idx + n1).reduce((acc, el) => acc + el, 0);
}
...
function findFirstSingleChar(str) {
  ...
  return letters.filter((el) => str.indexOf(el) === str.lastIndexOf(el))[0];
}
...
function getIntervalString(a, b, isStartIncluded, isEndIncluded) {
  const minEl = `${isStartIncluded ? '[' : '('}${Math.min(a, b)}`;
  const maxEl = `${Math.max(a, b)}${isEndIncluded ? ']' : ')'}`;
  return `${minEl}, ${maxEl}`;
}
...
function getDigitalRoot(num) {
  ...
  return getDigitalRoot(num.toString().split('').reduce((acc, el) => acc + +el, 0));
}
